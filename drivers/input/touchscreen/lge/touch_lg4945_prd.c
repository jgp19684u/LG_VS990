...
static void log_file_size_check(struct device *dev)
{
	...
	case 0:
		...
		break;
	case 1:
		fname = "/mnt/sdcard/touch_self_mfts.txt";
		break;
	default:
...
static int sdcard_spec_file_read(struct device *dev)
{
	int ret = 0;
	int fd = 0;
	char *path[2] = { "/mnt/sdcard/pplus_limit.txt",
		"/mnt/sdcard/pplus_limit_mfts.txt" };
	int mfts_mode = 0;
...
static int ic_exception_check(struct device *dev, char *buf)
{
	...
	if ((lge_get_boot_mode() > LGE_BOOT_MODE_NORMAL) || mfts_mode > 0) {
		if (d->fw.revision < 2 ||
				d->fw.revision > 3 ||
				d->fw.version[0] != 1) {
...
int prd_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
