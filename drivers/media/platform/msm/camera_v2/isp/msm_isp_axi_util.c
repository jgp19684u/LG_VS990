	...
	struct msm_vfe_axi_stream *stream_info, int32_t dual_vfe_sync)
{
	...
	for (i = 0; i < stream_info->num_planes; i++) {
		 ...
		 */
		if ((axi_data->stream_info[i].stream_src < RDI_INTF_0) &&
			vfe_dev->is_split && vfe_dev->pdev->id == ISP_VFE1 &&
			dual_vfe_sync) {
			...
			if (!dual_vfe_res->vfe_base[ISP_VFE0] ||
				...
				for (vfe_id = 0; vfe_id < MAX_VFE; vfe_id++) {
					vfe_dev->hw_info->vfe_ops.axi_ops.
						...
						stream_info->wm[i], enable_wm);
				}
			}
		} else if (!vfe_dev->is_split ||
			(axi_data->stream_info[i].stream_src >= RDI_INTF_0 &&
			axi_data->stream_info[i].stream_src <= RDI_INTF_2) ||
			!dual_vfe_sync) {
			vfe_dev->hw_info->vfe_ops.axi_ops.
