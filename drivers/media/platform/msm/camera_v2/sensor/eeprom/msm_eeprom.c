...
/* LGE: OV5670 verify eeprom checksum */
#define WIDE_AWB_5100K_CHECKSUM_ADDR 0x06
#define WIDE_AWB_3000K_CHECKSUM_ADDR 0x0E
#define WIDE_LSC_5100K_CHECKSUM_ADDR 0x384
#define WIDE_LSC_4000K_CHECKSUM_ADDR 0x6FA
#define WIDE_LSC_3100K_CHECKSUM_ADDR 0xA70

#define WIDE_AWB_5100K_CHECKSUM_START 0x00
#define WIDE_AWB_5100K_CHECKSUM_END 0x05

#define WIDE_AWB_3000K_CHECKSUM_START 0x08
#define WIDE_AWB_3000K_CHECKSUM_END 0x0D

#define WIDE_LSC_5100K_CHECKSUM_START 0x10
#define WIDE_LSC_5100K_CHECKSUM_END 0x383

#define WIDE_LSC_4000K_CHECKSUM_START 0x386
#define WIDE_LSC_4000K_CHECKSUM_END 0x6F9

#define WIDE_LSC_3100K_CHECKSUM_START 0x6FC
#define WIDE_LSC_3100K_CHECKSUM_END 0xA6F

#define NOR_AWB_5100K_CHECKSUM_ADDR 0xA78
#define NOR_AWB_3000K_CHECKSUM_ADDR 0xA80
#define NOR_LSC_5100K_CHECKSUM_ADDR 0xDF6
#define NOR_LSC_4000K_CHECKSUM_ADDR 0x116C
#define NOR_LSC_3100K_CHECKSUM_ADDR 0x14E2

#define NOR_AWB_5100K_CHECKSUM_START 0xA72
#define NOR_AWB_5100K_CHECKSUM_END 0xA77

#define NOR_AWB_3000K_CHECKSUM_START 0xA7A
#define NOR_AWB_3000K_CHECKSUM_END 0xA7F

#define NOR_LSC_5100K_CHECKSUM_START 0xA82
#define NOR_LSC_5100K_CHECKSUM_END 0xDF5

#define NOR_LSC_4000K_CHECKSUM_START 0xDF8
#define NOR_LSC_4000K_CHECKSUM_END 0x116B

#define NOR_LSC_3100K_CHECKSUM_START 0x116E
#define NOR_LSC_3100K_CHECKSUM_END 0x14E1
...
static int verify_eeprom_checksum(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	if ( checksum1 == checksum_calc1 && checksum2 == checksum_calc2) {
		...
		rc = -1;
		pr_err("%s: Fail AWB_5100K : calc= %d, checksum= %d\n", __func__,
			checksum_calc1, checksum1);
		pr_err("%s: Fail AWB_3000K : calc= %d, checksum= %d\n", __func__,
			checksum_calc2, checksum2);
	}
	...
	if ( checksum1 == checksum_calc1 && checksum2 == checksum_calc2 &&
		...
		rc = -1;
		pr_err("%s: Fail LSC_5100K : calc= %d, checksum= %d\n", __func__,
			checksum_calc1, checksum1);
		pr_err("%s:Fail LSC_3000K : calc= %d, checksum= %d\n", __func__,
			checksum_calc2, checksum2);
		pr_err("%s:Fail LSC_4000K : calc= %d, checksum= %d\n", __func__,
			checksum_calc3, checksum3);
	}
...
#ifdef CONFIG_OV5670W
static int verify_ov5670_eeprom_checksum(struct msm_eeprom_ctrl_t *e_ctrl)
{
	int i, checksum1 = 0, checksum2 = 0, checksum3 = 0;
	int checksum_calc1 = 0, checksum_calc2 = 0, checksum_calc3 = 0;
	int rc = 0;

	checksum1= (e_ctrl->cal_data.mapdata[WIDE_AWB_5100K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[WIDE_AWB_5100K_CHECKSUM_ADDR];
	for (i = WIDE_AWB_5100K_CHECKSUM_START; i <= WIDE_AWB_5100K_CHECKSUM_END; i++)
		checksum_calc1 += e_ctrl->cal_data.mapdata[i];

	CDBG("%s:WIDE_AWB_5100K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc1, checksum1);

	checksum2 = (e_ctrl->cal_data.mapdata[WIDE_AWB_3000K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[WIDE_AWB_3000K_CHECKSUM_ADDR];
	for (i = WIDE_AWB_3000K_CHECKSUM_START; i <= WIDE_AWB_3000K_CHECKSUM_END; i++)
		checksum_calc2 += e_ctrl->cal_data.mapdata[i];

	CDBG("%s:WIDE_AWB_3000K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc2, checksum2);
	if ( checksum1 == checksum_calc1 && checksum2 == checksum_calc2) {
		rc = 0;
	} else {
		rc = -1;
		pr_err("%s: Fail WIDE_AWB_5100K : calc= %d, checksum= %d\n", __func__,
			checksum_calc1, checksum1);
		pr_err("%s: Fail WIDE_AWB_3000K : calc= %d, checksum= %d\n", __func__,
			checksum_calc2, checksum2);
	}
	checksum_calc1 = checksum_calc2 = 0;

	checksum1= (e_ctrl->cal_data.mapdata[NOR_AWB_5100K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[NOR_AWB_5100K_CHECKSUM_ADDR];
	for (i = NOR_AWB_5100K_CHECKSUM_START; i <= NOR_AWB_5100K_CHECKSUM_END; i++)
		checksum_calc1 += e_ctrl->cal_data.mapdata[i];

	CDBG("%s:NOR_AWB_5100K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc1, checksum1);

	checksum2 = (e_ctrl->cal_data.mapdata[NOR_AWB_3000K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[NOR_AWB_3000K_CHECKSUM_ADDR];
	for (i = NOR_AWB_3000K_CHECKSUM_START; i <= NOR_AWB_3000K_CHECKSUM_END; i++)
		checksum_calc2 += e_ctrl->cal_data.mapdata[i];

	CDBG("%s:NOR_AWB_3000K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc2, checksum2);
	if ( checksum1 == checksum_calc1 && checksum2 == checksum_calc2) {
		rc = 0;
	} else {
		rc = -1;
		pr_err("%s: Fail NOR_AWB_5100K : calc= %d, checksum= %d\n", __func__,
			checksum_calc1, checksum1);
		pr_err("%s: Fail NOR_AWB_3000K : calc= %d, checksum= %d\n", __func__,
			checksum_calc2, checksum2);
	}
	checksum_calc1 = checksum_calc2 = 0;

	checksum1 = (e_ctrl->cal_data.mapdata[WIDE_LSC_5100K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[WIDE_LSC_5100K_CHECKSUM_ADDR];
	for (i = WIDE_LSC_5100K_CHECKSUM_START; i <= WIDE_LSC_5100K_CHECKSUM_END; i++) {
		checksum_calc1 += e_ctrl->cal_data.mapdata[i];
		checksum_calc1 = checksum_calc1 & 0xFFFF;
	}

	CDBG("%s:WIDE_LSC_5100K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc1, checksum1);
	checksum2 = (e_ctrl->cal_data.mapdata[WIDE_LSC_4000K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[WIDE_LSC_4000K_CHECKSUM_ADDR];
	for (i = WIDE_LSC_4000K_CHECKSUM_START; i <= WIDE_LSC_4000K_CHECKSUM_END; i++) {
		checksum_calc2 += e_ctrl->cal_data.mapdata[i];
		checksum_calc2 = checksum_calc2 & 0xFFFF;
	}

	CDBG("%s:WIDE_LSC_4000K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc2, checksum2);
	checksum3 = (e_ctrl->cal_data.mapdata[WIDE_LSC_3100K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[WIDE_LSC_3100K_CHECKSUM_ADDR];
	for (i = WIDE_LSC_3100K_CHECKSUM_START; i <= WIDE_LSC_3100K_CHECKSUM_END; i++) {
		checksum_calc3 += e_ctrl->cal_data.mapdata[i];
		checksum_calc3 = checksum_calc3 & 0xFFFF;
	}

	CDBG("%s:WIDE_LSC_3100K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc3, checksum3);
	...
	if ( checksum1 == checksum_calc1 && checksum2 == checksum_calc2 &&
		...
		rc = -1;
		pr_err("%s: Fail WIDE_LSC_5100K : calc= %d, checksum= %d\n", __func__,
			checksum_calc1, checksum1);
		pr_err("%s:Fail WIDE_LSC_3000K : calc= %d, checksum= %d\n", __func__,
			checksum_calc2, checksum2);
		pr_err("%s:Fail WIDE_LSC_4000K : calc= %d, checksum= %d\n", __func__,
			checksum_calc3, checksum3);
	}
	checksum_calc1 = checksum_calc2 = checksum_calc3 = 0;

	checksum1 = (e_ctrl->cal_data.mapdata[NOR_LSC_5100K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[NOR_LSC_5100K_CHECKSUM_ADDR];
	for (i = NOR_LSC_5100K_CHECKSUM_START; i <= NOR_LSC_5100K_CHECKSUM_END; i++) {
		checksum_calc1 += e_ctrl->cal_data.mapdata[i];
		checksum_calc1 = checksum_calc1 & 0xFFFF;
	}

	CDBG("%s:NOR_LSC_5100K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc1, checksum1);
	checksum2 = (e_ctrl->cal_data.mapdata[NOR_LSC_4000K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[NOR_LSC_4000K_CHECKSUM_ADDR];
	for (i = NOR_LSC_4000K_CHECKSUM_START; i <= NOR_LSC_4000K_CHECKSUM_END; i++) {
		checksum_calc2 += e_ctrl->cal_data.mapdata[i];
		checksum_calc2 = checksum_calc2 & 0xFFFF;
	}

	CDBG("%s:NOR_LSC_4000K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc2, checksum2);
	checksum3 = (e_ctrl->cal_data.mapdata[NOR_LSC_3100K_CHECKSUM_ADDR + 1] * 256)
		+ e_ctrl->cal_data.mapdata[NOR_LSC_3100K_CHECKSUM_ADDR];
	for (i = NOR_LSC_3100K_CHECKSUM_START; i <= NOR_LSC_3100K_CHECKSUM_END; i++) {
		checksum_calc3 += e_ctrl->cal_data.mapdata[i];
		checksum_calc3 = checksum_calc3 & 0xFFFF;
	}

	CDBG("%s:NOR_LSC_3100K checksum_calc = %d, checksum = %d\n", __func__,
		checksum_calc3, checksum3);
	...
	if ( checksum1 == checksum_calc1 && checksum2 == checksum_calc2 &&
		...
		rc = -1;
		pr_err("%s: Fail NOR_LSC_5100K : calc= %d, checksum= %d\n", __func__,
			checksum_calc1, checksum1);
		pr_err("%s:Fail NOR_LSC_3000K : calc= %d, checksum= %d\n", __func__,
			checksum_calc2, checksum2);
		pr_err("%s:Fail NOR_LSC_4000K : calc= %d, checksum= %d\n", __func__,
			checksum_calc3, checksum3);
	}
	return rc;
}
#endif // CONFIG_OV5670W
			 ...
			 const struct i2c_device_id *id)
{
	...
	for (j = 0; j < e_ctrl->cal_data.num_data; j++)
		CDBG("memory_data[0x%X] = 0x%X\n", j,
			e_ctrl->cal_data.mapdata[j]);

	e_ctrl->is_supported |= msm_eeprom_match_crc(&e_ctrl->cal_data);

#ifdef CONFIG_OV5670W
	if (verify_ov5670_eeprom_checksum(e_ctrl) < 0) {
		pr_err("%s: %s : verify eeprom data, id = 0x%x, ver = 0x%x\n",
			__func__, e_ctrl->eboard_info->eeprom_name, e_ctrl->cal_data.mapdata[0x1504],
				e_ctrl->cal_data.mapdata[0x1506]);
		pr_err("%s: %s : eeprom data checksum failed!\n", __func__,
			e_ctrl->eboard_info->eeprom_name);
	} else {
		pr_info("%s: %s : eeprom data checksum success!\n", __func__,
