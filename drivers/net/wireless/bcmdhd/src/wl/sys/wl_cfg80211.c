...
#ifdef PASSIVE_SCAN_ENABLE
	if(cfg->active_scan)
		params->scan_type = 0;
	else
		params->scan_type = 1; // WL_SCANFLAGS_PASSIVE;
#else
	params->scan_type = 0;
#endif
	params->nprobes = -1;
...
#ifdef PASSIVE_SCAN_ENABLE
void
wl_set_scan_mode(bool scan_mode)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
	if(scan_mode){ 	// 1 -> active scan
		WL_ERR(("ACTIVE SCAN MODE ENABLE\n"));
		cfg->active_scan = true;
	}
	else{		// 0 -> passive scan
		WL_ERR(("PASSIVE SCAN MODE ENABLE\n"));
