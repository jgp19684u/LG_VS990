...
typedef enum vzw_chg_state {
	VZW_NO_CHARGER,
	VZW_NORMAL_CHARGING,
#ifdef CONFIG_LGE_PM_INCOMPATIBLE_CHARGER_WITH_CHARGING
...
static int set_charger_control_current(int limit, int requester)
{
	...
	if (chgr_contr->qc20.is_qc20 && chgr_contr->qc20.is_highvol) {
		...
		if ( (batt_volt >= QC20_BATTERY_VOLTAGE_THR || (chgr_contr->is_cc_step_charging && batt_volt >= QC20_BATTERY_VOLTAGE_THR_LOW)) &&
	...
	if (chgr_contr->is_usb) {
		...
		if (chgr_contr->current_status == FAST_CHARGE_NORMAL)
			...
			chgr_contr->current_ibat_max = chgr_contr->current_ibat_lcd_off;
...
#else
#define VZW_SLOW_CHARGER_CHECK_COUNT 5
#endif
...
void changed_by_batt_psy(void)
{
	...
	if (vzw_aicl_complete) {
		...
		if (chgr_contr->vzw_under_current_count < VZW_SLOW_CHARGER_CHECK_COUNT) {
			...
			chgr_contr->vzw_chg_mode = VZW_NOT_CHARGING;
			pr_cc(PR_INFO, "VZW_NOT_CHARGING STATE\n");
			return;
		}
	}
#else
	...
	pr_cc(PR_INFO, "Set VZW_CHG_STATE = %d :"\
			" 0=NO_CHAGER,1=NORMAL, 2=INCOMPATIBLE_CHARGER,"\
