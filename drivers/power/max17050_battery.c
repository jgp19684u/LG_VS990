...
bool max17050_battery_full_info_print(void)
{
...
#ifdef CONFGIG_LGE_PM_MAX17050_DUMP
	int i;
#endif
	int vfocv_1;
	int vfocv_2;
	...
	if(battery_full_cap >= 3200) {
		pr_max17050(PR_ERR, "%s : [CMP] full cap before rewirete : full_cap = %d\n",
		...
		pr_max17050(PR_ERR, "%s : [CMP] full cap rewirete : full_cap = %d\n",
				__func__, battery_full_cap);
	}
		...
		if (cut_off_count <= 1) {
			...
			msecs_to_jiffies(MAX17050_POLLING_PERIOD_5));
				if (ref->soc_calc_cut_off_flag < 1) {
					...
					pr_max17050(PR_DEBUG, "%s : POLLING_FAKE_SOC : resd soc flag = %d\n",
...
void max17050_recalculate_soc(int before_vfocv)
{
	...
	msleep(350);
	/* Clear MiscCFG */
	max17050_write_reg(max17050_i2c_client, MAX17050_MISC_CFG, 0xFBFB);
		...
		int before_vfocv, int before_cell)
{
	...
	if (por_state == 0 && before_cell == cell_info
			&& before_soc > 1) {
		if ((current_soc - before_soc <= 5 &&
			current_soc - before_soc >= -5) ||
			(current_vfocv - before_vfocv <= tolerance_hi &&
...
static int max17050_new_custom_model_write(void)
{
	...
	pr_max17050(PR_INFO, "%s :[CMP] recal_flag = %d creset_flag = %d\n", __func__,
			read_recal_flag, read_reset_flag);
