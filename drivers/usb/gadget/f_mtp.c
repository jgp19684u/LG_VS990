...
#ifdef CONFIG_USB_G_LGE_ANDROID
bool mtp_skip_write = false;
module_param(mtp_skip_write, bool, S_IRUGO | S_IWUSR);
#endif
...
static void receive_file_work(struct work_struct *data)
{
	...
	while (count > 0 || write_req) {
		...
		dev->perf.receive_time =
				ktime_add(dev->perf.receive_time, diff);
		dev->perf.t_receive_time =
				ktime_add(dev->perf.t_receive_time, diff);
#endif
	}
		...
		size_t count, loff_t *ppos)
{
	...
	file_ready_rtime =
		ktime_to_ms(ktime_sub(ktime_sub(dev->perf.last_end_rtime,
		dev->perf.first_start_rtime), dev->perf.t_send_time));
	file_ready_wtime =
		ktime_to_ms(ktime_sub(ktime_sub(dev->perf.last_end_wtime,
	...
	t_wtemp = t_receive_time + file_ready_wtime ?
		DIV_ROUND_UP_ULL(t_wbytes, t_receive_time + file_ready_wtime)
							* 1000 / 1024 : 0;
	t_rtemp = t_send_time ?
		DIV_ROUND_UP_ULL(t_rbytes, t_send_time + file_ready_rtime)
							* 1000 / 1024 : 0;
