...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_EXTENDED_PANEL)
#define DEFAULT_MDP_TRANSFER_TIME 15000
#else
		...
		bool force_send)
{
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_EXTENDED_PANEL)
	if (force_send || !mdss_rect_cmp(c_roi, &roi) || pinfo->lge_pan_info.recalc_roi == 1) {
#else
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_EXTENDED_PANEL)
		if(pinfo->lge_pan_info.recalc_roi == 1) {
			if(p_roi->y == 0 && p_roi->h == 2720) {
				p_roi->y = 160;
				p_roi->h -= 160;
				pr_debug("recalc roi for NULL kick off %s: ndx=%d x=%d y=%d w=%d h=%d\n",
					__func__, ctrl->ndx, p_roi->x,
					p_roi->y, p_roi->w, p_roi->h);
			}
		}
	...
	if(pinfo->lge_pan_info.recalc_roi == 1) {
                p_roi->y = 0;
		p_roi->h = 2720;
		pinfo->lge_pan_info.recalc_roi = 0;
	}
#endif
end:
	return 0;
