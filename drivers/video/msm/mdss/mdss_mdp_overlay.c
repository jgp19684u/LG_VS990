...
// border fill change case
// 1. U2 -> U3 Partial
	if (skip_partial_update) {
		if((mfd->panel_info->type == MIPI_CMD_PANEL) &&
			(mfd->panel_info->lge_pan_info.cur_panel_mode == LGE_PANEL_MODE_U2 &&
			mfd->panel_info->lge_pan_info.skip_border_fill)) {
			l_roi = (struct mdss_rect){0, 160,
					ctl->mixer_left->width,
					ctl->mixer_left->height-160};
			if (ctl->mixer_right) {
					r_roi = (struct mdss_rect) {0, 160,
					ctl->mixer_right->width,
					ctl->mixer_right->height-160};
			}
		}else {
			l_roi = (struct mdss_rect){0, 0,
					ctl->mixer_left->width,
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_EXTENDED_PANEL)
	mfd->panel_info->lge_pan_info.skip_border_fill = 0;
	mfd->panel_info->lge_pan_info.recalc_roi = 0;
#endif
