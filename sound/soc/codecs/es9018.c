...
struct es9018_reg es9018_init_register[] = {
	...
	{ ESS9018_THD_COMP,		0x00 }, 	//GPIO2: output '0' to keep 9602 in Power-down mode
	{ ESS9018_2ND_HCOMP2,		0xff },  //test
	{ ESS9018_3RD_HCOMP2,		0xff },  //test
};
...
static int sabre_cf_num = 2;  //current custom filter num
...
struct es9018_regmap {
	...
	{ "FILTERADDR",			ESS9018_FILTER_ADDR, 1 },
	{ "FILTERCOEF",			ESS9018_FILTER_COEF, 1 },
	{ "FILTERCONT",			ESS9018_FILTER_CONT, 1 },
...

{
	...
	u8 rv;
	if(sabre_cf_num > 3) {
		rc = es9018_write_reg(g_es9018_priv->i2c_client, ESS9018_FILTER_CONT, 0x00);
		switch(sabre_cf_num) {
			case 4:
				rv = 0x80;
	...
	rc = es9018_write_reg(g_es9018_priv->i2c_client, ESS9018_FILTER_CONT, rv);
	if (rc < 0)
		return rc;

	rv = es9018_read_reg(g_es9018_priv->i2c_client, ESS9018_FILTER_ADDR-1);
	...
	for (i = 0; i < count_stage1 ; i++) { // Program Stage 1 filter coefficients
		...
		i2c_smbus_write_block_data(g_es9018_priv->i2c_client, ESS9018_FILTER_ADDR-1, 4, value);
		coeff++;
	}
	...
	es9018_write_reg(g_es9018_priv->i2c_client, ESS9018_FILTER_ADDR-1, rv);
...
static struct snd_kcontrol_new es9018_digital_ext_snd_controls[] = {
	...
	SOC_SINGLE_EXT("HIFI Custom Filter", SND_SOC_NOPM, 0, 6, 0, //current 0~3 filter num 4~6 built - in(fast - slow - min)
