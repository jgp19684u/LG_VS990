...
#ifdef CONFIG_SND_SOC_ES9018_COMPARATOR
static irqreturn_t wcd9xxx_mech_plug_detect_restore_irq(int irq, void *data)
{
	struct wcd9xxx_mbhc *mbhc = data;
	int r = IRQ_HANDLED;

	pr_debug("%s entered  mbhc->mbhc_cfg->mbhc_detect_restore =%d\n",__func__,mbhc->mbhc_cfg->mbhc_detect_restore);
...
static void comparator_detecting_mode(struct wcd9xxx_mbhc *mbhc)
{
	int ret=0;
	...
	pr_debug("%s: entered \n", __func__);
	if(mbhc->mbhc_cfg->mbhc_detect_restore_irq){
	pr_debug("%s: mbhc->mbhc_cfg->mbhc_detect_restore_irq=%d \n", __func__,mbhc->mbhc_cfg->mbhc_detect_restore_irq);
		gpio_set_value(mbhc->mbhc_cfg->mbhc_det_gpio, 0);
			usleep_range(plug_det->t_mic_pid, plug_det->t_mic_pid +
						WCD9XXX_USLEEP_RANGE_MARGIN_US);
		ret=request_irq(mbhc->mbhc_cfg->mbhc_detect_restore_irq,
				wcd9xxx_mech_plug_detect_restore_irq,
				IRQF_TRIGGER_FALLING,
				"mbhc_restore",
				mbhc);
		if (ret) {
			pr_err("%s: Failed to request mbhc_detect_restore_irq %d\n",
					 __func__,mbhc->mbhc_cfg->mbhc_detect_restore_irq);
		} else {
			ret = enable_irq_wake(mbhc->mbhc_cfg->mbhc_detect_restore_irq);
			if (ret)
				pr_err("%s: Failed to enable wake up irq %d\n",
					 __func__,mbhc->mbhc_cfg->mbhc_detect_restore_irq);
		}
	}
	snd_soc_update_bits(mbhc->codec, WCD9XXX_A_MBHC_INSERT_DETECT2,0xC0, 0x00);//auto pull down disable
	mbhc->micbias_enable_cb(mbhc->codec, true, mbhc->mbhc_cfg->anc_micbias);
	if(mbhc->hph_status==SND_JACK_HEADPHONE){
		pr_debug("%s mbhc->hph_status=%d mic bias enable for 3-pole \n",__func__,mbhc->hph_status);
...
static void lge_set_switch_device_for_hidden_aux(struct wcd9xxx_mbhc *mbhc, int state)
{
	...
	{ //LGE UPDATE L impedance is higher than 50k ohm
	...
	{ //LGE UPDATE L impedance is 100 ~ 50k ohm
		mbhc->sdev.name = LGE_SWITCH_NAME_ADVANCED;
	}
	else
...
static void lge_set_switch_device(struct wcd9xxx_mbhc *mbhc, int state)
{
	...
	{ //LGE UPDATE L impedance is 100 ~ 400 ohm
		mbhc->sdev.name = LGE_SWITCH_NAME_ADVANCED;
#ifdef CONFIG_SND_SOC_ES9018_COMPARATOR
		comparator_detecting_mode(mbhc);
#endif
	}
	else
	{ //LGE UPDATE L impedance is under 100 ohm
...
#ifdef CONFIG_SND_SOC_ES9018_COMPARATOR
	if(mbhc->mbhc_cfg->mbhc_det_gpio){
		if(gpio_get_value(mbhc->mbhc_cfg->mbhc_det_gpio))
			free_irq(mbhc->mbhc_cfg->mbhc_detect_restore_irq,mbhc);
	}
#endif
