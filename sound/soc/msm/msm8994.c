...
static struct wcd9xxx_mbhc_config mbhc_cfg = {
	...
	.hph_aud_sw = -1,
#endif
#if defined (CONFIG_SND_SOC_HPH_WA)||(defined (CONFIG_SND_SOC_ES9018_COMPARATOR))
	.mbhc_det_gpio=0,
#ifdef CONFIG_SND_SOC_ES9018_COMPARATOR
	.mbhc_detect_restore=0,
	.mbhc_detect_restore_irq=0,
#endif
#endif
...
#undef S
	...
	btn_low[7] = 2006;
	btn_high[7] = 2007;
#elif defined(CONFIG_SND_SOC_HPH_WA)
	btn_low[0] = 10;
	btn_high[0] = 110;
	btn_low[1] = 111;
	btn_high[1] = 190;
...
#ifdef CONFIG_SND_SOC_ES9018_COMPARATOR
	ret = of_get_named_gpio(spdev->dev.of_node,
					   "dac,mbhc-restore-gpio", 0);
	if (ret >= 0) {
		mbhc_cfg.mbhc_detect_restore= ret;
		gpio_request(mbhc_cfg.mbhc_detect_restore, "mbhc_detect_restore");
		gpio_direction_input(mbhc_cfg.mbhc_detect_restore);
		mbhc_cfg.mbhc_detect_restore_irq=gpio_to_irq(mbhc_cfg.mbhc_detect_restore);
	}
	else{
		pr_err("%s:  setting failed mbhc restore IRQ",__func__);
	}
#endif
#ifdef CONFIG_SND_EAR_MIC_SWITCH
        plug_gpio = of_get_named_gpio(spdev->dev.of_node,
